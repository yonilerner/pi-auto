...
For top-level `&&` / `||` chains that mix review-only and normal commands, pi-auto reviews the original command once, runs review-only segments bare, and wraps the other segments in the OS sandbox. For example, `gh auth status && ./safe-looking-script.sh` runs `gh auth status` review-only and `./safe-looking-script.sh` sandboxed. If a sandboxed segment is denied, pi-auto does not retry the whole chain outside the sandbox, because that would also unsandbox the normal segments. Compound chains where every command matches the configured prefix still run as one review-only command; `gh auth status && gh pr list` matches `[["gh"]]`.
